...
\begin{center}
	\begin{tikzpicture}[>=latex, remember picture]
		...
		\draw[->]  (decode.355)         -| node[right, align=center] {enqueue\\instruction} node[above right,pos=1] {\scriptsize +} (reorder_buffer);
		...
		\draw[->]  (write_back.185)     -| node[below, align=center, pos=0.25] {next pc} ($(fetch.west)-(7,0)$) -- (fetch);
