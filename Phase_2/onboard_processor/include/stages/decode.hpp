...
public:
	DecodeStage();
	#ifdef DBG_SYNTH
	void interface(FetchToDecode& from_fetch, DecodeToFetch* to_fetch, DecodeToIssue* to_issue, DecodeToCommit* to_commit, DecodeStatus* status);
	#else
	void interface(FetchToDecode& from_fetch, DecodeToFetch* to_fetch, DecodeToIssue* to_issue, DecodeToCommit* to_commit);
	#endif
};